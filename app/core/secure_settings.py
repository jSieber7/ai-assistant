...
class SecureSettingsManager:
    ...
    def _init_encryption(self):
        ...
        machine_id = os.environ.get(
            "COMPUTERNAME", os.environ.get("HOSTNAME", "default")
        )
    ...
    def _get_default_settings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "external_services": {
                ...
                "searxng": {"secret_key": "", "url": "http://searxng:8080"},
            },
           "system_config": {
        ...
        for category, values in self.settings.items():
            ...
            for key, value in values.items():
                if any(
                    ...
                    for sensitive in ["key", "password", "secret", "auth"]
                ):
                    if value and len(str(value)) > 4:
                        masked_settings[category][key] = (
                            f"{str(value)[:4]}{'*' * (len(str(value)) - 4)}"
                        )
                    else:
        ...
        try:
            ...
            if provider == "openai_compatible":
                ...
                with httpx.Client(timeout=10) as client:
                    ...
                    response = client.get(
                        "https://api.jina.ai/v1/rerank", headers=headers
                    )
                    return response.status_code in [
                        200,
                        400,
                        422,
                    ]  # 422 is OK for empty request
