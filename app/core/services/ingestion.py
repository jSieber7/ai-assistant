...
class IngestionService:
        ...
        try:
            ...
            if not collection_name:
                if not self.vector_client:
                    raise ValueError("Vector client required for automatic collection creation")
                
                session_id = str(uuid.uuid4())
                collection_name = await self.vector_client.create_temporary_collection(session_id)
            ...
            logger.info(f"Ingestion completed: {len(documents)} documents → {len(chunks)} chunks "
        ...
        try:
            ...
            for i in range(0, len(chunks), self.batch_size):
                ...
                for chunk, embedding in zip(batch, embedding_vectors):
                    ...
                    chunk.metadata["embedding_model"] = getattr(self.embeddings, 'model', 'unknown')
    ...
    async def create_temporary_collection(self, session_id: Optional[str] = None) -> str:
        """
    ...
    def _update_stats(self, documents_count: int, chunks_count: int, execution_time: float):
        ...
        self._stats["batches_processed"] += (chunks_count + self.batch_size - 1) // self.batch_size
        self._stats["total_ingestion_time"] += execution_time
        self._stats["avg_chunk_processing_time"] = (
            self._stats["total_ingestion_time"] / max(self._stats["chunks_created"], 1)
        )
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_chunks_per_document": (
                self._stats["chunks_created"] / max(self._stats["documents_processed"], 1)
            ),
            "config": {
