...
class DynamicToolExecutor:
    ...
    def __init__(self, tool_registry: ToolRegistry):
        ...
        self._task_tool_mapping = {
            ...
            TaskType.SCRAPE: ["scrape", "crawl", "extract", "content", "web", "firecrawl"],
    ...
    async def _select_tools(self, request: TaskRequest) -> List[BaseTool]:
        ...
        if len(selected_tools) < request.max_tools:
            ...
            for tool in relevant_tools:
                ...
                if self._is_tool_relevant_to_task(tool, request.task_type, task_keywords):
                    selected_tools.append(tool)
    ...
    def _is_tool_relevant_to_task(self, tool: BaseTool, task_type: TaskType, task_keywords: List[str]) -> bool:
        """
    ...
    async def _execute_tools(self, request: TaskRequest, tools: List[BaseTool]) -> List[ToolResult]:
        """
        ...
        for tool in tools:
            try:
                ...
                if not tool.validate_parameters(**tool_params):
                    logger.warning(f"Invalid parameters for tool '{tool.name}': {tool_params}")
                ...
                logger.info(f"Executed tool '{tool.name}' with success: {result.success}")
    ...
    def _map_parameters(self, request: TaskRequest, tool: BaseTool) -> Dict[str, Any]:
        ...
        if request.task_type == TaskType.SEARCH:
            ...
            if "results_count" not in tool_params and "results_count" in tool.parameters:
                tool_params["results_count"] = tool.parameters["results_count"].get("default", 10)
    ...
    def _extract_url_from_query(self, query: str, context: Dict[str, Any]) -> Optional[str]:
        """
    ...
    async def _process_results(self, request: TaskRequest, tool_results: List[ToolResult]) -> Any:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_execution_time = sum(r.execution_time for r in self._execution_history) / total_executions
    ...
    def _get_most_used_tools(self) -> List[Dict[str, Any]]:
        ...
        return [{"tool_name": name, "usage_count": count} for name, count in sorted_tools[:10]]