...
class TestConfigurationValidation:
    ...
    def test_validate_port_range(self):
        ...
        with patch.dict("os.environ", {"PORT": "99999"}):  # Invalid port
            ...
            try:
                settings = Settings()
                # If it doesn't raise an error, the port might be accepted
                # or converted to a valid value
                assert settings.port in range(1, 65536)
            except ValueError:
    ...
    def test_validate_url_settings(self):
        ...
        with patch.dict("os.environ", {"OLLAMA_BASE_URL": "http://custom:11434"}):
            settings = Settings()
            assert settings.ollama_settings.base_url == "http://custom:11434"
