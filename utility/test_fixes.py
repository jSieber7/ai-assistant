...
def main():
    ...
    success, stdout, stderr = run_test(
        "tests/unit/test_config.py::TestSettings::test_settings_defaults"
    )
    if success:
    ...
    success, stdout, stderr = run_test(
        "tests/integration/test_docker.py::TestDockerIntegration::test_service_status"
    )
    if success:
    ...
    success, stdout, stderr = run_test(
        "tests/integration/test_docker.py::TestDockerIntegration::test_service_health"
    )
    if success:
    ...
    success, stdout, stderr = run_test(
        "tests/integration/test_docker.py::TestDockerIntegration::test_application_endpoints"
    )
    if success:
