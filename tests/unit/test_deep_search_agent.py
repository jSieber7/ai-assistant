...
class MockLLM(Runnable):
    ...
    async def ainvoke(self, input_data, config=None, **kwargs):
        ...
        if isinstance(input_data, str):
            ...
            prompt = input_data.get("input", "") or input_data.get("prompt", "") or input_data.get("user_query", "")
        else:
...
class TestDeepSearchAgent:
    ...
    async def test_process_documents(self, deep_search_agent):
        ...
        documents = [
            Document(
                page_content="This is a long document about artificial intelligence. " * 50,
    ...
    async def test_retrieve_and_synthesize(self, deep_search_agent):
        ...
        mock_milvus_client = AsyncMock()
        mock_milvus_client.similarity_search.return_value = [
            (Document(page_content="AI development 1", metadata={"source": "test1"}), 0.9),
            (Document(page_content="AI development 2", metadata={"source": "test2"}), 0.8)
        ]
        deep_search_agent.milvus_client = mock_milvus_client
