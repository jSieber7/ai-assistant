...
from app.core.config.multi_writer_config import get_multi_writer_config, is_multi_writer_enabled
...
class MultiContentOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.jinja_processor = JinjaProcessor(
            self.config["templates"]["template_dir"]
        )
    ...
    async def create_content(
        ...
        try:
            ...
            logger.info(f"Starting Stage 1: Source processing for workflow {workflow_id}")
            result["stages"]["source_processing"] = await self._stage_1_process_sources(sources)
            
            # Stage 2: Generate content with multiple writers
            logger.info(f"Starting Stage 2: Content generation for workflow {workflow_id}")
            result["stages"]["content_generation"] = await self._stage_2_generate_content(
                prompt, result["stages"]["source_processing"]["processed_content"], style_guide
            )
            
            # Stage 3: Quality checking and improvement
            logger.info(f"Starting Stage 3: Quality checking for workflow {workflow_id}")
            result["stages"]["quality_checking"] = await self._stage_3_quality_checking(
                result["stages"]["content_generation"], quality_threshold, max_iterations
            )
            
            # Stage 4: Template rendering
            logger.info(f"Starting Stage 4: Template rendering for workflow {workflow_id}")
            result["stages"]["template_rendering"] = await self._stage_4_template_rendering(
                result["stages"]["quality_checking"]["best_content"], template_name
            )
            
            # Final result
            result["final_output"] = result["stages"]["template_rendering"]["rendered_content"]
            ...
            if save_to_db:
                try:
                    ...
                    for content in result["stages"]["content_generation"]["writer_results"]:
                        content["workflow_id"] = workflow_id
                        await mongodb_client.save_content(content)
                    
                    # Save check results
                    for check in result["stages"]["quality_checking"]["checking_history"]:
                    ...
                    await mongodb_client.update_workflow(workflow_id, {
                        "status": "completed",
            ...
            if save_to_db:
                try:
                    ...
                    logger.error(f"Failed to update workflow status in database: {str(db_error)}")
        
        return result
    
    async def _stage_1_process_sources(self, sources: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _stage_2_generate_content(
        ...
        writer_results = await self.writer_orchestrator.generate_multiple_versions(
            prompt, combined_content, writer_ids=default_writers, style_guide=style_guide
        )
...
async def create_multi_content_orchestrator(config: Dict[str, Any] = None) -> MultiContentOrchestrator:
