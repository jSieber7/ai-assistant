...
class SynthesisService:
    ...
    async def _prepare_documents(self, documents: List[Document]) -> List[Document]:
        ...
        logger.debug(
            f"Prepared {len(prepared_docs)} documents for synthesis "
    ...
    async def _generate_answer(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Fallback answer generation also failed: {str(fallback_error)}"
                )
    ...
    def _update_stats(self, documents_count: int, synthesis_time: float):
        ...
        self._stats["avg_synthesis_time"] = self._stats["total_synthesis_time"] / max(
            self._stats["syntheses_performed"], 1
        )
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_documents_per_synthesis": (
                self._stats["total_documents_processed"]
                / max(self._stats["syntheses_performed"], 1)
            ),
            "config": {
                "max_context_length": self.max_context_length,
