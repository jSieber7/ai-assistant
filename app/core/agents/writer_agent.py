...
class WriterAgent(BaseAgent):
    ...
    def __init__(self, writer_id: str, specialty: str, model: str = "claude-3.5-sonnet"):
    ...
    def _calculate_confidence(self, content: str, source_content: Dict[str, Any]) -> float:
        ...
        if '\n\n' in content and any(line.strip().endswith(':') for line in content.split('\n')):
            score += 0.1
        
        # Check for key points integration
        key_points = source_content.get("key_points", [])
        integrated_points = sum(1 for point in key_points if point.lower() in content.lower())
