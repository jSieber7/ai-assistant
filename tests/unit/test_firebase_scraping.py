...
class TestFirebaseScraperTool:
    ...
    @pytest.mark.asyncio
    async def test_execute_with_http_scraping(self, scraper_tool, mock_httpx, mock_firebase):
    ...
    @pytest.mark.asyncio
    async def test_execute_with_selenium(self, scraper_tool, mock_selenium, mock_firebase):
        """Test Selenium-based scraping"""
        # Mock page source and BeautifulSoup
        mock_selenium['driver'].page_source = '<html><title>Test</title><body>Content</body></html>'
...
class TestFirebaseScraperAgent:
    ...
    def mock_llm(self):
        ...
        mock_llm.agenerate.return_value.generations = [[mock_response]]
        return mock_llm

    @pytest.fixture
    def scraper_agent(self, mock_llm):
        """Create a Firebase scraper agent instance"""
        return FirebaseScraperAgent(llm=mock_llm)
    ...
    async def test_url_extraction(self, scraper_agent):
        ...
        query = "Please scrape content from https://example.com and https://test-site.org"
...
class TestFirebaseIntegration:
    ...
    async def test_firebase_initialization(self):
        ...
        # Mock Firebase settings
        with patch.object(settings.firebase_settings, 'enabled', True), \
             ...
             patch.object(settings.firebase_settings, 'client_email', 'test@example.com'):
