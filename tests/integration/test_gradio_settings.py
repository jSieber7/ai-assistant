...
class TestGradioSettingsInterface(unittest.TestCase):
    ...
    def test_validate_api_key_function(self):
        ...
        with patch('app.ui.settings_page.secure_settings', self.mock_manager):
            ...
            self.mock_manager.validate_api_key.assert_called_once_with("openai_compatible", "test_key")
    ...
    def test_update_multi_writer_settings(self):
        ...
        with patch('app.ui.settings_page.secure_settings', self.mock_manager):
            ...
            self.assertEqual(settings["mongodb_connection_string"], "mongodb://test:27017")
    ...
    def test_export_settings(self):
        ...
        with patch('app.ui.settings_page.secure_settings', self.mock_manager):
            ...
            self.mock_manager.export_settings.assert_called_once_with(include_secrets=False)
            
            # Check result
            exported_data = json.loads(result)
            self.assertIn("llm_providers", exported_data)
            self.assertEqual(
                exported_data["llm_providers"]["openai_compatible"]["enabled"],
                True
            )
    ...
    def test_import_settings(self):
        ...
        with patch('app.ui.settings_page.secure_settings', self.mock_manager):
            ...
            self.mock_manager.import_settings.assert_called_once_with(settings_json, merge=True)
    ...
    def test_create_settings_page(self, mock_blocks):
        ...
        with patch('app.ui.settings_page.gr.Markdown'), \
...
class TestGradioSettingsWorkflow(unittest.TestCase):
    ...
    def test_complete_settings_workflow(self):
        ...
        settings_manager.set_category("llm_providers", {
            "openai_compatible": {
                "enabled": True,
        ...
        settings_manager.set_category("external_services", {
            "jina_reranker": {
                "enabled": True,
        ...
        settings_manager.set_category("system_config", {
            "tool_system_enabled": False,
            "agent_system_enabled": True,
            "preferred_provider": "openai_compatible"
        })
        ...
        self.assertEqual(
            settings_manager.get_setting("llm_providers", "openai_compatible", {}).get("api_key"),
            "test_openai_key"
        )
        self.assertEqual(
            settings_manager.get_setting("external_services", "jina_reranker", {}).get("api_key"),
            "test_jina_key"
        )
        self.assertFalse(
    ...
    def test_api_key_validation_workflow(self, mock_client):
        ...
        # 2. Save the API key
        settings_manager.set_setting("llm_providers", "openai_compatible", "api_key", "test_key")
        
        # 3. Verify the key was saved
        saved_key = settings_manager.get_setting("llm_providers", "openai_compatible", "api_key")
