            ...
            async with session.get(search_url, params=params) as response:
                if response.status == 200:
                    ...
                    if results.get("results"):
                        print(f"✅ SearXNG search returned {len(results['results'])} results")
                    ...
                    print(f"❌ SearXNG search failed with status {response.status}: {error_text}")
...
async def test_ai_assistant_searxng_tool() -> Dict[str, Any]:
    ...
    try:
        ...
        if result.get("results"):
            print(f"✅ AI Assistant SearXNG tool returned {len(result['results'])} results")
...
async def main():
    ...
    if direct_result["success"]:
        ...
        print(f"❌ Direct SearXNG access: FAILED - {direct_result.get('error', 'Unknown error')}")
    
    if tool_result["success"]:
        print("✅ AI Assistant SearXNG tool: PASSED")
    else:
        print(f"❌ AI Assistant SearXNG tool: FAILED - {tool_result.get('error', 'Unknown error')}")
