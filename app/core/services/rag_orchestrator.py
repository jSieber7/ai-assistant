...
class RAGOrchestrator:
    ...
    async def process_query(
        ...
        try:
            logger.info(f"Starting RAG pipeline for query: {user_query} (session: {session_id})")
            ...
            if not scraped_documents:
                ...
                result["answer"] = "I couldn't find relevant information for your query. Please try rephrasing or providing more specific terms."
    ...
    async def _step_query_processing(
        ...
        try:
            optimized_query = await self.query_service.generate_search_query(user_query, context)
            ...
            logger.info(f"Query optimization completed: '{user_query}' → '{optimized_query}'")
    ...
    async def _step_search_and_scraping(
        ...
        try:
            ...
            logger.info(f"Search and scraping completed: {len(scraped_documents)} documents")
    ...
    async def _step_ingestion(
        ...
        try:
            ...
            collection_name = await self.ingestion_service.create_temporary_collection(session_id)
            ...
            logger.info(f"Ingestion completed: {ingestion_result['chunks_ingested']} chunks in {collection_name}")
    ...
    async def _step_synthesis(
        ...
        try:
            ...
            logger.info(f"Synthesis completed: {len(synthesis_result.get('answer', ''))} characters")
    ...
    async def _finalize_result(self, result: Dict[str, Any], start_time: float) -> Dict[str, Any]:
        ...
        self._stats["avg_execution_time"] = (
            self._stats["total_execution_time"] / max(self._stats["total_queries_processed"], 1)
        )
        
        logger.info(f"RAG pipeline completed in {result['execution_time']:.2f}s. "
    ...
    def get_orchestrator_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self._stats["successful_queries"] / max(self._stats["total_queries_processed"], 1)
            ),
            "total_execution_time": self._stats["total_execution_time"],
            "avg_execution_time": self._stats["avg_execution_time"],
            "temporary_collections_created": self._stats["temporary_collections_created"],
            "temporary_collections_cleaned": self._stats["temporary_collections_cleaned"],
