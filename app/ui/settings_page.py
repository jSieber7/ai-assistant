...
def update_llm_provider_settings(
    ...
    try:
        ...
        openai_config = {
            ...
            "base_url": openai_base_url.strip() if openai_base_url else "https://openrouter.ai/api/v1",
            "default_model": openai_default_model.strip() if openai_default_model else "anthropic/claude-3.5-sonnet",
            "provider_name": openai_provider_name.strip() if openai_provider_name else "",
        ...
        ollama_config = {
            ...
            "base_url": ollama_base_url.strip() if ollama_base_url else "http://localhost:11434",
            "default_model": ollama_default_model.strip() if ollama_default_model else "llama2",
            "timeout": ollama_timeout,
            "max_retries": ollama_max_retries,
            "temperature": ollama_temperature,
            "max_tokens": ollama_max_tokens if ollama_max_tokens and ollama_max_tokens > 0 else None,
            "streaming": ollama_streaming
        }
        
        # Save to secure storage
        secure_settings.set_category("llm_providers", {
...
def update_external_service_settings(
    ...
    try:
        ...
        firecrawl_config = {
            ...
            "docker_url": firecrawl_docker_url.strip() if firecrawl_docker_url else "http://firecrawl-api:3002",
            "bull_auth_key": firecrawl_bull_auth_key.strip() if firecrawl_bull_auth_key else "",
        ...
        jina_config = {
            ...
            "model": jina_model.strip() if jina_model else "jina-reranker-v2-base-multilingual",
        ...
        # Save to secure storage
        secure_settings.set_category("external_services", {
...
def update_multi_writer_settings(
    ...
    try:
        multi_writer_config = {
            ...
            "mongodb_connection_string": mongodb_connection_string.strip() if mongodb_connection_string else "mongodb://localhost:27017",
            "mongodb_database_name": mongodb_database_name.strip() if mongodb_database_name else "multi_writer_system"
        }
...
def create_settings_page() -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        gr.Markdown("Configure your AI assistant settings securely. API keys and sensitive data are encrypted and stored locally.")
        ...
        with gr.Tabs():
                    ...
                    with gr.Column(scale=1):
                        ...
                        with gr.Group(elem_classes=["settings-section"]):
                            openai_enabled = gr.Checkbox(
                                ...
                                value=current_settings.get("llm_providers", {}).get("openai_compatible", {}).get("enabled", True)
                            )
                            ...
                            openai_base_url = gr.Textbox(
                                label="Base URL",
                                value=current_settings.get("llm_providers", {}).get("openai_compatible", {}).get("base_url", "https://openrouter.ai/api/v1")
                            )
                            openai_default_model = gr.Textbox(
                                label="Default Model",
                                value=current_settings.get("llm_providers", {}).get("openai_compatible", {}).get("default_model", "anthropic/claude-3.5-sonnet")
                            )
                            openai_provider_name = gr.Textbox(
                                label="Provider Name (optional)",
                                placeholder="e.g., OpenRouter, Anthropic",
                                value=current_settings.get("llm_providers", {}).get("openai_compatible", {}).get("provider_name", "")
                            )
                            with gr.Row():
                                openai_timeout = gr.Number(
                                    label="Timeout (seconds)",
                                    value=current_settings.get("llm_providers", {}).get("openai_compatible", {}).get("timeout", 30),
                                    minimum=1,
                                    maximum=300
                                )
                                openai_max_retries = gr.Number(
                                    label="Max Retries",
                                    value=current_settings.get("llm_providers", {}).get("openai_compatible", {}).get("max_retries", 3),
                                    minimum=0,
                                    maximum=10
                                )
                            
                            with gr.Row():
                                validate_openai_btn = gr.Button("🔍 Validate API Key", size="sm")
                    ...
                    with gr.Column(scale=1):
                        ...
                        with gr.Group(elem_classes=["settings-section"]):
                            ollama_enabled = gr.Checkbox(
                                label="Enable Ollama",
                                value=current_settings.get("llm_providers", {}).get("ollama", {}).get("enabled", True)
                            )
                            ollama_base_url = gr.Textbox(
                                label="Base URL",
                                value=current_settings.get("llm_providers", {}).get("ollama", {}).get("base_url", "http://localhost:11434")
                            )
                            ollama_default_model = gr.Textbox(
                                label="Default Model",
                                value=current_settings.get("llm_providers", {}).get("ollama", {}).get("default_model", "llama2")
                            )
                            with gr.Row():
                                ollama_timeout = gr.Number(
                                    label="Timeout (seconds)",
                                    value=current_settings.get("llm_providers", {}).get("ollama", {}).get("timeout", 30),
                                    minimum=1,
                                    maximum=300
                                )
                                ollama_max_retries = gr.Number(
                                    label="Max Retries",
                                    value=current_settings.get("llm_providers", {}).get("ollama", {}).get("max_retries", 3),
                                    minimum=0,
                                    maximum=10
                                )
                            ollama_temperature = gr.Slider(
                                ...
                                maximum=2.0,
                                step=0.1,
                                value=current_settings.get("llm_providers", {}).get("ollama", {}).get("temperature", 0.7)
                            )
                            ollama_max_tokens = gr.Number(
                                label="Max Tokens (0 for unlimited)",
                                value=current_settings.get("llm_providers", {}).get("ollama", {}).get("max_tokens", 0) or 0,
                                minimum=0,
                                maximum=8000
                            )
                            ollama_streaming = gr.Checkbox(
                                label="Enable Streaming",
                                value=current_settings.get("llm_providers", {}).get("ollama", {}).get("streaming", True)
                            )
                
                update_llm_btn = gr.Button("💾 Save LLM Provider Settings", variant="primary")
                llm_status = gr.Textbox(label="Status", interactive=False, elem_classes=["success"])
                    ...
                    with gr.Column():
                        ...
                        with gr.Group(elem_classes=["settings-section"]):
                            firecrawl_enabled = gr.Checkbox(
                                ...
                                value=current_settings.get("external_services", {}).get("firecrawl", {}).get("enabled", False)
                            )
                            firecrawl_docker_url = gr.Textbox(
                                label="Docker URL",
                                value=current_settings.get("external_services", {}).get("firecrawl", {}).get("docker_url", "http://firecrawl-api:3002")
                            )
                            ...
                            firecrawl_scraping_enabled = gr.Checkbox(
                                ...
                                value=current_settings.get("external_services", {}).get("firecrawl", {}).get("scraping_enabled", True)
                            )
                            with gr.Row():
                                firecrawl_max_concurrent_scrapes = gr.Number(
                                    label="Max Concurrent Scrapes",
                                    value=current_settings.get("external_services", {}).get("firecrawl", {}).get("max_concurrent_scrapes", 5),
                                    minimum=1,
                                    maximum=20
                                )
                                firecrawl_scrape_timeout = gr.Number(
                                    label="Scrape Timeout (seconds)",
                                    value=current_settings.get("external_services", {}).get("firecrawl", {}).get("scrape_timeout", 60),
                                    minimum=10,
                                    maximum=300
                                )
                    ...
                    with gr.Column():
                        ...
                        with gr.Group(elem_classes=["settings-section"]):
                            jina_enabled = gr.Checkbox(
                                ...
                                value=current_settings.get("external_services", {}).get("jina_reranker", {}).get("enabled", False)
                            )
                            ...
                            jina_url = gr.Textbox(
                                label="Reranker URL",
                                value=current_settings.get("external_services", {}).get("jina_reranker", {}).get("url", "http://jina-reranker:8080")
                            )
                            jina_model = gr.Textbox(
                                label="Model",
                                value=current_settings.get("external_services", {}).get("jina_reranker", {}).get("model", "jina-reranker-v2-base-multilingual")
                            )
                            with gr.Row():
                                jina_timeout = gr.Number(
                                    label="Timeout (seconds)",
                                    value=current_settings.get("external_services", {}).get("jina_reranker", {}).get("timeout", 30),
                                    minimum=1,
                                    maximum=300
                                )
                                jina_cache_ttl = gr.Number(
                                    label="Cache TTL (seconds)",
                                    value=current_settings.get("external_services", {}).get("jina_reranker", {}).get("cache_ttl", 3600),
                                    minimum=0,
                                    maximum=86400
                                )
                            jina_max_retries = gr.Number(
                                label="Max Retries",
                                value=current_settings.get("external_services", {}).get("jina_reranker", {}).get("max_retries", 3),
                                minimum=0,
                                maximum=10
                            )
                            
                            with gr.Row():
                                validate_jina_btn = gr.Button("🔍 Validate API Key", size="sm")
                ...
                with gr.Group(elem_classes=["settings-section"]):
                    with gr.Row():
                        ...
                        searxng_url = gr.Textbox(
                            label="SearXNG URL",
                            value=current_settings.get("external_services", {}).get("searxng", {}).get("url", "http://searxng:8080")
                        )
                
                update_services_btn = gr.Button("💾 Save External Service Settings", variant="primary")
                services_status = gr.Textbox(label="Status", interactive=False, elem_classes=["success"])
            ...
            with gr.TabItem("⚙️ System Configuration"):
                    ...
                    with gr.Column():
                        ...
                        with gr.Group(elem_classes=["settings-section"]):
                            tool_system_enabled = gr.Checkbox(
                                ...
                                value=current_settings.get("system_config", {}).get("tool_system_enabled", True)
                            )
                            agent_system_enabled = gr.Checkbox(
                                label="Enable Agent System",
                                value=current_settings.get("system_config", {}).get("agent_system_enabled", True)
                            )
                            enable_fallback = gr.Checkbox(
                                label="Enable Provider Fallback",
                                value=current_settings.get("system_config", {}).get("enable_fallback", True)
                            )
                            debug_mode = gr.Checkbox(
                                label="Debug Mode",
                                value=current_settings.get("system_config", {}).get("debug", True)
                            )
                    ...
                    with gr.Column():
                        ...
                        with gr.Group(elem_classes=["settings-section"]):
                            preferred_provider = gr.Dropdown(
                                ...
                                value=current_settings.get("system_config", {}).get("preferred_provider", "openai_compatible")
                            )
                            environment = gr.Dropdown(
                                label="Environment",
                                choices=["development", "production", "testing"],
                                value=current_settings.get("system_config", {}).get("environment", "development")
                            )
                            with gr.Row():
                                host = gr.Textbox(
                                    label="Host",
                                    value=current_settings.get("system_config", {}).get("host", "127.0.0.1")
                                )
                                port = gr.Number(
                                    label="Port",
                                    value=current_settings.get("system_config", {}).get("port", 8000),
                                    minimum=1,
                                    maximum=65535
                                )
                ...
                update_system_btn = gr.Button("💾 Save System Settings", variant="primary")
                system_status = gr.Textbox(label="Status", interactive=False, elem_classes=["success"])
            ...
            with gr.TabItem("📝 Multi-Writer System"):
                ...
                with gr.Group(elem_classes=["settings-section"]):
                    multi_writer_enabled = gr.Checkbox(
                        ...
                        value=current_settings.get("multi_writer", {}).get("enabled", False)
                    )
                    mongodb_connection_string = gr.Textbox(
                        label="MongoDB Connection String",
                        value=current_settings.get("multi_writer", {}).get("mongodb_connection_string", "mongodb://localhost:27017")
                    )
                    mongodb_database_name = gr.Textbox(
                        label="Database Name",
                        value=current_settings.get("multi_writer", {}).get("mongodb_database_name", "multi_writer_system")
                    )
                
                update_multi_writer_btn = gr.Button("💾 Save Multi-Writer Settings", variant="primary")
                multi_writer_status = gr.Textbox(label="Status", interactive=False, elem_classes=["success"])
            ...
            with gr.TabItem("📥 Import/Export"):
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        import_btn = gr.Button("📥 Import Settings", variant="secondary")
        ...
        validate_openai_btn.click(
            validate_api_key,
            inputs=[gr.Textbox(value="openai_compatible", visible=False), openai_api_key],
        ...
        update_llm_btn.click(
            ...
            inputs=[
                openai_enabled, openai_api_key, openai_base_url, openai_default_model,
                openai_provider_name, openai_timeout, openai_max_retries,
                ollama_enabled, ollama_base_url, ollama_default_model,
                ollama_timeout, ollama_max_retries, ollama_temperature,
                ollama_max_tokens, ollama_streaming
            ],
            outputs=[llm_status]
        )
        ...
        update_services_btn.click(
            ...
            inputs=[
                firecrawl_enabled, firecrawl_docker_url, firecrawl_bull_auth_key,
                firecrawl_scraping_enabled, firecrawl_max_concurrent_scrapes,
                firecrawl_scrape_timeout, jina_enabled, jina_api_key,
                jina_url, jina_model, jina_timeout, jina_cache_ttl,
                jina_max_retries, searxng_secret_key, searxng_url
            ],
        ...
        update_system_btn.click(
            ...
            inputs=[
                tool_system_enabled, agent_system_enabled, preferred_provider,
                enable_fallback, debug_mode, host, port, environment, secret_key
            ],
        ...
        update_multi_writer_btn.click(
            ...
            inputs=[multi_writer_enabled, mongodb_connection_string, mongodb_database_name],
            outputs=[multi_writer_status]
        )
        
        export_btn.click(
            export_settings,
            inputs=[include_secrets],
