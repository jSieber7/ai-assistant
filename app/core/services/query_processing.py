...
class QueryProcessingService:
    ...
    async def generate_search_query(self, user_query: str, context: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        try:
            ...
            if not optimized_query or len(optimized_query) < 3:
                logger.warning(f"Query optimization failed, using original query: {user_query}")
                return user_query
            
            logger.info(f"Generated optimized search query: '{optimized_query}' from '{user_query}'")
    ...
    async def batch_generate_queries(self, user_queries: list[str], context: Optional[Dict[str, Any]] = None) -> list[str]:
        """
    ...
    def validate_query(self, query: str) -> Dict[str, Any]:
        ...
        if any(operator in query.lower() for operator in ["site:", "filetype:", "intitle:", "inurl:"]):
        ...
        if not any(word in query.lower() for word in ["what", "how", "why", "when", "where", "who"]):
            if "?" not in query:
                analysis["suggestions"].append("Consider adding question words for better results")
