"""
LangGraph-based collaborative checking system for multi-writer/checker system.
...
class CollaborativeCheckerState(TypedDict):
    """State for the LangGraph collaborative checker"""
    content: str
    checkers: List[LangGraphCheckerAgent]
    context: Optional[Dict[str, Any]]
    individual_assessments: List[CheckerAssessment]
...
class LangGraphCollaborativeChecker:
    """LangGraph-based orchestrator for collaborative checking between checker agents"""
    ...
    def _create_workflow(self) -> StateGraph:
        """Create the LangGraph workflow for collaborative checking"""
        workflow = StateGraph(CollaborativeCheckerState)

        # Add nodes
        workflow.add_node("gather_assessments", self._gather_assessments)
        workflow.add_node("identify_disputes", self._identify_disputes)
        workflow.add_node("resolve_disputes", self._resolve_disputes)
        workflow.add_node("build_consensus", self._build_consensus)

        # Add edges
        workflow.set_entry_point("gather_assessments")
        workflow.add_edge("gather_assessments", "identify_disputes")
        workflow.add_edge("identify_disputes", "resolve_disputes")
        workflow.add_edge("resolve_disputes", "build_consensus")
        workflow.add_edge("build_consensus", END)

        return workflow.compile()

    async def conduct_collaborative_review(
        self, content: str, checkers: List[LangGraphCheckerAgent], context: Dict[str, Any] = None
    ) -> ConsensusResult:
        """
        ...
        try:
            ...
            if result.get("error"):
                logger.error(f"Collaborative review failed: {result['error']}")
            ...
            if consensus_result:
                ...
                logger.error("No consensus result produced")
                return self._fallback_consensus(content, checkers, context)
    ...
    async def _gather_assessments(self, state: CollaborativeCheckerState) -> CollaborativeCheckerState:
        ...
        state["individual_assessments"] = assessments
        return state

    async def _get_checker_assessment(
        self,
        checker: LangGraphCheckerAgent,
    ...
    async def _identify_disputes(self, state: CollaborativeCheckerState) -> CollaborativeCheckerState:
    ...
    async def _resolve_disputes(self, state: CollaborativeCheckerState) -> CollaborativeCheckerState:
        ...
        state["resolved_disputes"] = resolved_disputes
        state["unresolved_disputes"] = unresolved_disputes
        return state
    ...
    async def _resolve_single_dispute(
        ...
        try:
            ...
            # Parse resolution
            resolution = self._parse_dispute_resolution(response.content if hasattr(response, 'content') else str(response))
    ...
    async def _build_consensus(self, state: CollaborativeCheckerState) -> CollaborativeCheckerState:
        """Build consensus from all assessments and resolutions"""
        assessments = state["individual_assessments"]
        resolved_disputes = state["resolved_disputes"]
        unresolved_disputes = state["unresolved_disputes"]
        content = state["content"]
...
"""
    ...
    def _fallback_consensus(
        self, content: str, checkers: List[LangGraphCheckerAgent], context: Dict[str, Any]
    ) -> ConsensusResult:
