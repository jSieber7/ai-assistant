...
from app.core.config.multi_writer_config import is_multi_writer_enabled, get_multi_writer_config
...
class ContentRequest(BaseModel):
    ...
    sources: List[Dict[str, Any]] = Field(..., description="List of source URLs or content")
    style_guide: Optional[Dict[str, Any]] = Field(None, description="Style guide for content")
    template_name: str = Field("article.html.jinja", description="Template to use for rendering")
    quality_threshold: float = Field(70.0, description="Minimum quality threshold (0-100)")
    max_iterations: int = Field(2, description="Maximum iterations for quality improvement")
...
async def get_workflow_check_results(
    ...
    try:
        ...
        if not results:
            raise HTTPException(status_code=404, detail="No check results found for workflow")
...
async def get_config(_=Depends(check_multi_writer_enabled)):
    ...
    try:
        ...
        config["firecrawl"]["api_key"] = "***" if config["firecrawl"]["api_key"] else None
        config["mongodb"]["connection_string"] = "***" if config["mongodb"]["connection_string"] else None
