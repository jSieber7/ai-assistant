...
class MongoDBClient:
    ...
    def __init__(self, connection_string: str, database_name: str = "multi_writer_system"):
    ...
    async def get_workflow_check_results(self, workflow_id: str) -> List[Dict[str, Any]]:
    ...
    async def list_workflows(self, status: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def delete_workflow(self, workflow_id: str) -> bool:
        ...
        # Delete workflow
        workflow_result = await self.db.workflows.delete_one({"workflow_id": workflow_id})
        
        # Delete related content
        content_result = await self.db.content.delete_many({"workflow_id": workflow_id})
        
        # Delete related check results
        check_result = await self.db.check_results.delete_many({"workflow_id": workflow_id})
        
        return (
            workflow_result.deleted_count > 0 or
            content_result.deleted_count > 0 or
            check_result.deleted_count > 0
        )
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        completed_workflows = await self.db.workflows.count_documents({"status": "completed"})
        ...
        avg_quality_score = await self.db.workflows.aggregate([
            {"$match": {"status": "completed", "stages.quality_checking.best_score": {"$exists": true}}},
            {"$group": {"_id": None, "avg_score": {"$avg": "$stages.quality_checking.best_score"}}}
        ]).to_list(length=1)
        ...
        return {
            ...
            "average_quality_score": avg_quality_score[0]["avg_score"] if avg_quality_score else 0
        }
...
async def get_mongodb_client() -> MongoDBClient:
    ...
    if mongodb_client is None:
        ...
        database_name = getattr(settings, 'mongodb_database_name', 'multi_writer_system')
