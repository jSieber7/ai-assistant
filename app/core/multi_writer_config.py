...
class MultiWriterSettings(BaseSettings):
    ...
    available_writers: Dict[str, Dict[str, Any]] = {
        ...
        "creative_1": {"specialty": "creative", "model": "claude-3.5-sonnet"},
        "analytical_1": {"specialty": "analytical", "model": "gpt-4-turbo"},
    }
    ...
    available_checkers: Dict[str, Dict[str, Any]] = {
        ...
        "structure_1": {"focus_area": "structure", "model": "gpt-4-turbo"},
        "seo_1": {"focus_area": "seo", "model": "gpt-4-turbo"},
    }
...
def get_multi_writer_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "firecrawl": {
            "api_key": (
                multi_writer_settings.firecrawl_api_key.get_secret_value()
                if multi_writer_settings.firecrawl_api_key
                else None
            ),
...
def validate_multi_writer_config() -> List[str]:
    ...
    for writer_id in multi_writer_settings.default_writers:
        if writer_id not in multi_writer_settings.available_writers:
            issues.append(
                f"Default writer '{writer_id}' not found in available writers"
            )

    for checker_id in multi_writer_settings.default_checkers:
        if checker_id not in multi_writer_settings.available_checkers:
            issues.append(
                f"Default checker '{checker_id}' not found in available checkers"
            )
