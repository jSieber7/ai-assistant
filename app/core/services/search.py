...
class SearchService:
    ...
    async def search_and_scrape(
        ...
        try:
            ...
            self._stats["total_content_length"] += sum(len(doc.page_content) for doc in scraped_documents)
            
            execution_time = time.time() - start_time
            logger.info(f"Search and scrape completed in {execution_time:.2f}s. "
    ...
    async def _scrape_urls(self, urls: List[str], original_query: str) -> List[Document]:
        """
    ...
    async def _scrape_single_url(self, url: str, original_query: str) -> Optional[Document]:
        """
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_content_length": (
                self._stats["total_content_length"] / max(self._stats["successful_scrapes"], 1)
            ),
            "config": {
