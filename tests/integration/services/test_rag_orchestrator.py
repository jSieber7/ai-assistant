...
class TestRAGOrchestrator:
    ...
    def mock_embeddings(self):
        ...
        embeddings.aembed_documents = AsyncMock(return_value=[[0.1, 0.2, 0.3] for _ in range(5)])
    ...
    def mock_services(self, mock_llm, mock_embeddings):
        ...
        search_service.search_and_scrape = AsyncMock(return_value=[
            Document(page_content="Document 1 content", metadata={"source": "url1"}),
            Document(page_content="Document 2 content", metadata={"source": "url2"})
        ])
        
        ingestion_service = Mock(spec=IngestionService)
        ingestion_service.create_temporary_collection = AsyncMock(return_value="test_collection")
        ingestion_service.ingest_documents = AsyncMock(return_value={
            "success": True,
        ...
        retrieval_service.retrieve_and_rerank = AsyncMock(return_value=[
        ...
        synthesis_service.synthesize_answer = AsyncMock(return_value={
    ...
    async def test_process_query_success(self, orchestrator, mock_services):
        ...
        assert result["answer"] == "This is the synthesized answer based on the documents."
        ...
        mock_services["query_service"].generate_search_query.assert_called_once_with(user_query, None)
        mock_services["search_service"].search_and_scrape.assert_called_once()
        mock_services["ingestion_service"].create_temporary_collection.assert_called_once()
    ...
    async def test_process_query_with_context(self, orchestrator, mock_services):
        ...
        mock_services["query_service"].generate_search_query.assert_called_once_with(user_query, context)
    ...
    async def test_process_query_synthesis_failure(self, mock_services):
        ...
        mock_services["synthesis_service"].synthesize_answer = AsyncMock(return_value={
    ...
    async def test_process_query_with_auto_cleanup(self, mock_services):
        ...
        mock_services["ingestion_service"].drop_collection.assert_called_once_with("test_collection")
    ...
    def test_get_orchestrator_stats(self, orchestrator, mock_services):
        ...
        # Mock service stats
        mock_services["query_service"].get_service_stats.return_value = {"service": "query_stats"}
        mock_services["search_service"].get_service_stats.return_value = {"service": "search_stats"}
        mock_services["ingestion_service"].get_service_stats.return_value = {"service": "ingestion_stats"}
        mock_services["retrieval_service"].get_service_stats.return_value = {"service": "retrieval_stats"}
        mock_services["synthesis_service"].get_service_stats.return_value = {"service": "synthesis_stats"}
    ...
    async def test_step_query_processing_failure(self, mock_services):
        ...
        mock_services["query_service"].generate_search_query = AsyncMock(side_effect=Exception("Query processing failed"))
        ...
        pipeline_steps = result["pipeline_steps"]
        assert pipeline_steps["query_processing"]["success"] is True  # Fallback makes it successful
    ...
    async def test_create_default_orchestrator(self, mock_llm, mock_embeddings):
        ...
        with patch('app.core.services.rag_orchestrator.QueryProcessingService') as mock_query_service, \
             patch('app.core.services.rag_orchestrator.SearchService') as mock_search_service, \
             patch('app.core.services.rag_orchestrator.IngestionService') as mock_ingestion_service, \
             patch('app.core.services.rag_orchestrator.RetrievalService') as mock_retrieval_service, \
             patch('app.core.services.rag_orchestrator.SynthesisService') as mock_synthesis_service, \
             patch('app.core.services.rag_orchestrator.MilvusClient') as mock_milvus_client:
            ...
            mock_retrieval_service.assert_called_once_with(mock_client, mock_tool_registry)
