...
class TestSearchService:
    ...
    @pytest.mark.asyncio
    async def test_search_and_scrape_success(self, search_service, sample_search_results, sample_documents):
        """Test successful search and scrape."""
        query = "Python async programming"
        
        # Mock dynamic executor
        with patch.object(search_service.dynamic_executor, 'execute_task') as mock_execute:
            # Mock search result
            search_task_result = TaskResult(
                ...
                tool_results=[],
                execution_time=0.5,
                task_type=search_service.dynamic_executor._task_tool_mapping["search"][0]
            )
            ...
            mock_execute.side_effect = [search_task_result, scrape_task_result, scrape_task_result]
            ...
            assert result[1].metadata["source"] == "https://example.com/asyncio-tutorial"
    ...
    async def test_scrape_urls_only(self, search_service, sample_documents):
        ...
        urls = ["https://example.com/python-async", "https://example.com/asyncio-tutorial"]
    ...
    @pytest.mark.asyncio
    async def test_search_and_scrape_with_context(self, search_service, sample_search_results):
