...
class TestStandaloneScraper:
    ...
    async def test_scrape_url_execution_error(self, mock_scraper):
        ...
        mock_scraper._scraper.execute = AsyncMock(side_effect=Exception("Scraping failed"))
    ...
    async def test_extract_content_only_too_short(self, mock_scraper):
        ...
        content = await mock_scraper.extract_content_only("https://example.com", min_length=50)
...
class TestQuickScrape:
    ...
    async def test_quick_scrape_success(self):
        ...
        with patch('app.core.tools.standalone_scraper.settings') as mock_settings:
            ...
            with patch('app.core.tools.standalone_scraper.StandaloneScraper') as mock_scraper_class:
                ...
                mock_scraper.scrape_url.assert_called_once_with("https://example.com", None)
    ...
    async def test_quick_scrape_with_formats(self):
        ...
        with patch('app.core.tools.standalone_scraper.settings') as mock_settings:
            ...
            with patch('app.core.tools.standalone_scraper.StandaloneScraper') as mock_scraper_class:
                ...
                assert result == {"content": "test content"}
                mock_scraper.scrape_url.assert_called_once_with("https://example.com", ["markdown"])
    ...
    async def test_full_workflow(self):
        ...
        with patch('app.core.tools.standalone_scraper.settings') as mock_settings:
            ...
            scraper._scraper.execute.side_effect = [
                ...
                {"images": [{"url": "https://example.com/image.jpg"}]},  # For extract_images
                {"title": "Test Page", "description": "Test description"}  # For get_metadata
            ]
            
            scraper._scraper.batch_scrape = AsyncMock(return_value=[
                {"url": "https://example.com", "content": "Content 1"},
                {"url": "https://example.org", "content": "Content 2"}
            ])
