...
class MilvusClient:
    ...
    def _create_collection_schema(self) -> CollectionSchema:
        ...
        schema = CollectionSchema(
            fields=fields,
            description="Deep Search temporary collection"
        )
    ...
    async def create_temporary_collection(self, session_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            index_params = {
                ...
                "params": {"M": 8, "efConstruction": 64} if self.settings.index_type == "HNSW" else {}
            }
    ...
    async def ingest_documents(
        ...
        try:
            ...
            for i in range(0, len(documents), batch_size):
                ...
                logger.debug(f"Inserted batch of {len(batch)} documents into {collection_name}")
            
            # Flush to ensure data is persisted
            collection.flush()
            
            logger.info(f"Successfully ingested {ingested_count} documents into {collection_name}")
    ...
    async def similarity_search(
        ...
        try:
            ...
            logger.debug(f"Found {len(documents_with_scores)} results for query in {collection_name}")
            return documents_with_scores
            
        except Exception as e:
            logger.error(f"Failed to perform similarity search in {collection_name}: {str(e)}")
            raise
...
class MilvusVectorStore(LangChainMilvus):
    ...
    async def ingest_documents(self, collection_name: str, documents: List[Document]) -> int:
