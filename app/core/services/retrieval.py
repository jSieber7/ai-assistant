...
class RetrievalService:
    ...
    async def retrieve_and_rerank(
        ...
        try:
            ...
            if self.enable_reranking and len(retrieved_docs) > 1:
                reranked_docs = await self._rerank_documents(query, retrieved_docs, top_n)
            else:
            ...
            logger.info(f"Retrieval and reranking completed in {retrieval_time:.2f}s. "
        ...
        try:
            ...
            if search_params is None:
                search_params = {
                    ...
                    "params": {"ef": 64} if settings.milvus_settings.index_type == "HNSW" else {}
                }
            ...
            logger.debug(f"Retrieved {len(retrieved_docs)} documents from collection {collection_name}")
    ...
    async def _rerank_documents(
        ...
        try:
            ...
            logger.info(f"Reranking completed in {reranking_time:.2f}s. "
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_retrieval_time": (
                self._stats["total_retrieval_time"] / max(self._stats["retrievals_performed"], 1)
            ),
            "avg_reranking_time": (
                self._stats["total_reranking_time"] / max(self._stats["rerankings_performed"], 1)
            ),
            "reranking_success_rate": (
                (self._stats["rerankings_performed"] - self._stats["reranking_failures"]) /
    ...
    async def validate_collection(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            if not self.vector_client:
                return {
                    "valid": False,
                    "error": "No vector client configured"
                }
