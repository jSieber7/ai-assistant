...
def get_models_list() -> List[str]:
    ...
    try:
        ...
        return [f"{model.provider.value}:{model.name}" if model.provider.value not in ["openrouter", "openai_compatible"] else model.name for model in models]
    except Exception:
...
def get_providers_info() -> str:
    ...
    try:
        ...
        for provider in providers:
            ...
            is_default = " (Default)" if provider.provider_type == provider_registry._default_provider else ""
