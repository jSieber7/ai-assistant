...
async def test_deep_search_full_workflow():
    ...
    agent = DeepSearchAgent(tool_registry=tool_registry, llm=llm, embeddings=embeddings)
...
async def test_milvus_connection():
    ...
    try:
        ...
        ingested_count = await milvus_client.ingest_documents(
        ...
        results = await milvus_client.similarity_search(
            collection_name=collection_name, query="artificial intelligence", k=5
        )
...
async def test_searxng_and_firecrawl_integration():
    ...
    try:
        ...
        search_result = await searxng_tool.execute(
            query="artificial intelligence", results_count=3
        )
        ...
        scrape_result = await firecrawl_tool.execute(
            url=first_url, formats=["markdown"]
        )
        ...
        pytest.skip(
            f"SearXNG/Firecrawl integration test skipped (services may not be running): {e}"
        )
...
async def test_jina_reranker_integration():
    ...
    from app.core.tools.content.jina_reranker_tool import JinaRerankerTool
    ...
    try:
        ...
        if not result.success:
            assert (
                ...
                or "couldn't find" in result.response.lower()
            )
