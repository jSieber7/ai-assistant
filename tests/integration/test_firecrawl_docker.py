...
class TestFirecrawlDockerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_docker_health_check_success(self, docker_config, mock_health_response):
        """Test successful Docker health check"""
        tool = FirecrawlTool()
        
        with patch.object(tool._get_client(), 'get', return_value=mock_health_response):
    ...
    async def test_docker_health_check_failure(self, docker_config):
        ...
        with patch.object(tool._get_client(), 'get', side_effect=httpx.ConnectError("Connection failed")):
            is_healthy = await tool._check_docker_health()
            assert is_healthy is False
    
    @pytest.mark.asyncio
    async def test_docker_scraping_success(self, docker_config, mock_docker_response, mock_health_response):
        """Test successful scraping with Docker Firecrawl"""
        tool = FirecrawlTool()
        
        with patch.object(tool, '_check_docker_health', return_value=True), \
             patch.object(tool._get_client(), 'post', return_value=mock_docker_response):
    ...
    @pytest.mark.asyncio
    async def test_docker_scraping_with_fallback(self, docker_config, mock_docker_response, mock_health_response):
        ...
        with patch.object(tool, '_check_docker_health', return_value=False), \
             patch.object(tool._get_fallback_client', 'post', return_value=mock_docker_response):
    ...
    @pytest.mark.asyncio
    async def test_docker_agent_integration(self, docker_config, mock_docker_response, mock_health_response):
        """Test Firecrawl agent with Docker deployment"""
        # Mock LLM
        mock_llm = AsyncMock()
        mock_response = AsyncMock()
        mock_response.generations = [[
            AsyncMock(text='{"content_type": "general", "recommended_formats": ["markdown"], "extraction_targets": ["content"], "challenges": [], "recommendations": []}')
        ]]
    ...
    @pytest.mark.asyncio
    async def test_docker_batch_scraping(self, docker_config, mock_docker_response, mock_health_response):
